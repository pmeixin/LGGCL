...
#            x_cl = (1 - self.alpha) * torch.matmul(adj, x_cl) + self.alpha * x0 
    ...
    def suplabel_lossv6neg(self, z1: torch.Tensor, z2: torch.Tensor , mask: torch.Tensor, neg_mask: torch.Tensor, pos_mask: torch.Tensor, debias, mean_type: int = 1):
        if mean_type == 0:
    ...
    def cl_lossaug(self, z1: torch.Tensor, z2: torch.Tensor, mask: torch.Tensor, train_mask: torch.Tensor, labels, neg_mask, pos_mask,
