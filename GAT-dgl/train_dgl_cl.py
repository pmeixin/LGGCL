...
#training process
def train(epoch, model, optimizer, features, labels, idx_train, idx_val, idx_test, sample_size, debias,  data_aug, neg_type, pos_type, kk: int = 1):
    t = time.time()
    model.train()
    optimizer.zero_grad()
...
for i in range(times):
    ...
    for epoch in range(args.epochs):
        loss_e, acc_e, test_logits = train(epoch, model, optimizer, features, labels, idx_train, idx_val, idx_test, args.sample_size, args.debias, args.data_aug, args.neg_type, args.pos_type)
